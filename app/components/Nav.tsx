  ...
  return (
    <section className='max-w-[90%] mx-auto h-16 shadow-md flex items-center justify-between px-4 text-white mt-4 rounded-md bg-green-500 fixed top-0 left-0 right-0 z-99'>
      ...
      <div className='flex flex-row items-center gap-3'>
        ...
        {/* User Image */}
        <div className='relative'><Image src={user?.image || ''} alt='User' width={36} height={36} className='w-9 h-9 rounded-full cursor-pointer' onClick={() => setShowUserMenu(!showUserMenu)} /></div>
        <AnimatePresence>
          {showUserMenu && (
            <motion.div
              key="user-menu"
